...
cat >$NETIP <<-EOF
uci set network.lan.ipaddr='192.168.100.4'                      # IPv4 地址(openwrt后台地址)
uci set network.lan.netmask='255.255.255.0'                   # IPv4 子网掩码
uci set network.lan.gateway='192.168.100.1'                     # IPv4 网关
...
uci set system.@system[0].hostname='OpenWrt-7zht'              # 修改主机名称为OpenWrt-123
...
sed -i "s/OpenWrt /${7zht.com} compiled in $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ_PATH
